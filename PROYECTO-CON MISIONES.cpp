...
void inicializarMisiones() {
    insertarMision(&lista_misiones, "Primer Acierto", "Adivina el número a la primera.", "Adivinar a la primera", 2.00);
    insertarMision(&lista_misiones, "Tres Victorias", "Gana tres partidas.", "Alcanzar 3 victorias", 5.00);
    insertarMision(&lista_misiones, "Jugador Persistente", "Juega 5 partidas.", "Jugar 5 partidas", 3.00);
}
...
// Declarar funciones faltantes para el menú
void RankingJugadores(Jugador *perfiles) {
    cout << "\n--- Ranking de Jugadores ---\n";
    int pos = 1;
    Jugador *aux = perfiles;
    while(aux != nullptr){
        cout << pos++ << ". " << aux->alias << " - Victorias: " << aux->victorias << endl;
        aux = aux->prox;
    }
    cout << "----------------------------\n";
}

void VerPerfil(Jugador *perfiles) {
...
int main() {
    ...
    do {
        ...
        switch (opcion_logueo) {
            case 1:
                if (Vacio(perfiles)) {
                    cout << "No hay usuarios registrados. Regístrese primero." << endl;
                    break;
                }
                if (IniciarSesion(perfiles)) {
                    do {
                        MenuUsuario(perfiles);
                        if (!(cin >> opcion_usuario)) {
                            cin.clear();
                            cin.ignore(10000, '\n');
                            cout << "Entrada inválida. Intente de nuevo." << endl;
                            continue;
    ...
    } while (opcion_logueo != 0);

    LiberarMemoriaJugadores(perfiles);
    LiberarMemoriaMisiones(lista_misiones);
